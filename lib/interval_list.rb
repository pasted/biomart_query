...
attr_accessor :name, :exons, :read_group, :interval_list
...
@SQ	SN:GL000192.1	LN:547496"
	...
	def initialize(name, exons)
		begin
			self.name = name
			self.exons = exons
			self.read_group = "@RG	ID:#{self.name}	PL:ILLUMINA	LB:hg19"
		rescue
			puts "Error on assignment"
		end
	end

	def parse_list()
		self.interval_list = @@header + "\n"
		self.interval_list += self.read_group + "\n"
		self.exons.each do |this_exon|
			self.interval_list += this_exon.output_tsv() + "\n"
		end
